...
AWS.S3.prototype.uploadImage = function (fileInfo, uploadInfo, callback) {
    ...
    // 썸네일 경로
    const thumbnailPath = fileInfo.filePath + 'thumbnail';
    ...
    async.waterfall([
        ...
        // 파일 삭제
        (taskDone) => {
            for (var path in pathForDelete) {
                console.log('file delte :', path);
            }
            // TODO : 파일 삭제
            taskDone(null);
        }
...
const s3Param = {
    itemKey: 'idol/iu.jpg',
    thumbnailKey: 'thumbnail/iu.jpg'
};

s3.uploadImage(param, s3Param, (err, result) => {
    if (err) {
